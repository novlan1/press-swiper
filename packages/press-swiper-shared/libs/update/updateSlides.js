...
export default function updateSlides() {
  ...
  const shouldResetSlideSize = params.slidesPerView === 'auto' && params.breakpoints && Object.keys(params
    .breakpoints).filter(key => typeof params.breakpoints[key].slidesPerView !== 'undefined').length > 0;
  ...
  Array(...Array(slidesLength)).forEach(async (item, i) => {
    ...
    if (params.centeredSlides) {
      ...
      if ((index - Math.min(swiper.params.slidesPerGroupSkip, index)) % swiper.params
        .slidesPerGroup === 0) snapGrid.push(slidePosition);
      slidesGrid.push(slidePosition);
  ...
  if (params.spaceBetween !== 0) {
    let key = '';
    // #ifdef MP-BAIDU
    key = swiper.isHorizontal() && rtl ? 'marginLeft' : getDirectionLabel('marginRight');
    // #endif
    // #ifndef MP-BAIDU
    key = swiper.isHorizontal() && rtl ? 'margin-left' : getDirectionLabel('margin-right');
    // #endif
  ...
  if (params.centeredSlides && params.cssMode && !params.centeredSlidesBounds) {
    ...
    setCSSProperty(
      swiper.wrapperEl, '--swiper-centered-offset-after',
