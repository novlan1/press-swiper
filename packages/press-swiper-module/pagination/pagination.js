import classesToSelector from 'press-swiper-shared/shared/classes-to-selector.js';
...
export default function Pagination({
  ...
  function setSideBullets($bulletEl, position) {
    ...
    if (bullets[$bulletEl.index + position]) {
      bullets[$bulletEl.index + position].addPaginationItemClass(`${bulletActiveClass}-${position > 0 ? 'next' : 'prev'}`);
    }
    if (bullets[$bulletEl.index + (position > 0 ? position + 1 : position
				- 1)]) {
      bullets[$bulletEl.index + (position > 0 ? position + 1 : position - 1)].addPaginationItemClass(`${bulletActiveClass}-${position > 0 ? 'next' : 'prev'}-${position > 0 ? 'next' : 'prev'}`);
    }
  }
  ...
  function update() {
    ...
    if (
      ...
      bullets.forEach((item) => {
        item.removePaginationItemClass(['', '-next', '-next-next', '-prev', '-prev-prev', '-main']
        ...
        if (params.dynamicBullets) {
          ...
          if (swiper.params.loop) {
            if (bulletIndex >= bullets.length) {
              ...
              bullets[bullets.length - params.dynamicMainBullets - 1]
      ...
      for (let i = 0; i < numberOfBullets; i += 1) {
        ...
        // paginationHTML += 1;
          ...
          swiper.native.paginationContent.push({
            ...
            styleContent: {},
            addPaginationItemClass(value) {
              this.classContent = Array.from(new Set([...this.classContent,
                ...value.split(' '),
              ]));
            },
            removePaginationItemClass(value) {
              this.classContent = this.classContent.filter(item => !value.split(' ').includes(item));
            },
            setCss(value) {
              // vueNative['itemStyle'] = {
              // 	...vueNative['itemStyle'],
  ...
  function init() {
    ...
    if (params.clickable) {
      swiper.on('paginationItemClick', (_s, itemIndex) => {
