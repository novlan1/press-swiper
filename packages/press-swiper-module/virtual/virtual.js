  ...
  async function update(force) {
    ...
    if (force) {
      ...
      for (let i = previousFrom; i <= previousTo; i += 1) {
        if (i < from || i > to) {
          swiper.virtualList.splice(swiper.virtualList.findIndex(item => item == slides[i]), 1);

          swiper.virtualIndexList.splice(swiper.virtualIndexList.findIndex(item => item == i), 1);
