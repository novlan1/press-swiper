  ...
  >
    ...
    >
      ...
      <view
        :class="['swiper-wrapper']"
        :style="[wrapperStyle]"
        :swiper-transform="wxsTransform"
        :change:swiperTransform="zSwiperWxs.wxsTransformObserver"
        @click="onClickWrapper"
        @touchstart="zSwiperWxs.onTouchStartWxs"
        @touchmove="zSwiperWxs.onTouchMoveWxs"
        @touchend="zSwiperWxs.onTouchEndWxs"
        @touchcancel="zSwiperWxs.onTouchEndWxs"
      >
        <!-- #endif -->
        <slot />
        ...
        <template v-if="loopBlankShow">
          <z-swiper-item
            v-for="(item,index) in loopBlankNumber"
            :key="index"
          />
        </template>
      ...
      <template v-if="showIndicators">
        <view
          :class="['swiper-pagination',paginationClass]"
          :style="[paginationStyle]"
        >
          <template v-if="paginationType == 'bullets'">
            <view
              v-for="(item,index) in paginationContent"
              :key="index"
              :class="[item.classContent.join(' ')]"
              :style="[item.styleContent]"
              @click="paginationItemClick(index)"
            />
          </template>
          <template v-if="paginationType == 'fraction'">
            <text :class="paginationContent.currentClass">
              {{ paginationContent.text }}
            </text>/<text
              :class="paginationContent.totalClass"
            >
              {{ paginationContent.total }}
            </text>
          </template>
          <template v-if="paginationType == 'progressbar'">
            <text
              :class="paginationContent.progressbarFillClass"
              :style="[paginationContent.styleContent]"
            />
          </template>
        </view>
      </template>
      <template v-if="(showPrevButton||showPrevButtonSlot)">
        <view
          :class="['swiper-button-prev',prevClass]"
          @click="prevClick"
        >
          <view
            v-if="!showPrevButtonSlot"
      ...
      <template v-if="(showNextButton||showNextButtonSlot)">
        <view
          :class="['swiper-button-next',nextClass]"
          @click="nextClick"
        >
          <view
            v-if="!showNextButtonSlot"
      ...
      <template v-if="scrollbarShow">
        <view
          :class="['swiper-scrollbar',scrollbarClass]"
...
<script>
	import {
		getAllRect,
		getRect
	} from 'press-swiper-shared/libs/utils/utils.js';
	// vue2
	import {
		getParams
	} from 'press-swiper-shared/libs/vue2/get-params.js';
	import {
		initSwiper,
		mountSwiper
	} from 'press-swiper-shared/libs/vue2/init-swiper.js';
	import {
		needsScrollbar,
		needsNavigation,
		needsPagination,
		uniqueClasses,
		extend,
	} from 'press-swiper-shared/libs/vue2/utils.js';
	import {
		renderLoop,
		calcLoopedSlides
	} from 'press-swiper-shared/libs/vue2/loop.js';
	import {
		getChangedParams
	} from 'press-swiper-shared/libs/vue2/get-changed-params.js';
	import {
		updateSwiper
	} from 'press-swiper-shared/libs/vue2/update-swiper.js';
	import {
		renderVirtual,
		updateOnVirtualData
	} from 'press-swiper-shared/libs/vue2/virtual.js';
	//mixin
	import {
		ParentMixin
	} from 'press-swiper-shared/libs/mixins/relation.js';
...
@import "press-swiper-shared/libs/core.scss";
@import "press-swiper-shared/static/css/iconfont.css";
